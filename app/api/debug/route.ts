...
export async function GET() {
    ...
    const diagnostics: Record<string, any> = {
        ...
        cloudinary_configured: !!process.env.CLOUDINARY_URL,
    };
    ...
    // Test 2: Direct connection (uses DIRECT_URL)
    ...
    // Test 3: Cloudinary config test
    diagnostics.test_storage = { working: false, error: null as string | null };
    try {
        const config = cloudinary.config();
        if (config.cloud_name) {
            diagnostics.test_storage.working = true;
            diagnostics.test_storage.cloud_name = config.cloud_name;
        } else {
            diagnostics.test_storage.error = 'Cloudinary is not configured properly (missing cloud_name)';
        }
    } catch (error) {
